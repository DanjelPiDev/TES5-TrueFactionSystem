...
namespace NPE {
    ...
    * Faction and Armor Data
    ...
    std::unordered_map<RE::FormID, std::string> factionFormIDToTagMap = {
        ...
        {0x0002BF9A, "npeImperialFaction"},
        {0x00072834, "npeBladesFaction"},
        {0x00048362, "npeCompanionsFaction"},
        {0x0002816E, "npeDawnstarFaction"},
        {0x00028170, "npeFalkreathFaction"},
        {0x00043599, "npeForswornFaction"},
        {0x0002816C, "npeMarkarthFaction"},
        {0x0002816D, "npeMorthalFaction"},
        {0x000DEED6, "npeNightingaleFaction"},
        {0x0002816B, "npeRiftenFaction"},
        {0x00029DB0, "npeSolitudeFaction"},
        {0x00028849, "npeStormcloaksFaction"},
        {0x000267EA, "npeWhiterunFaction"},
        {0x000267E3, "npeWindhelmFaction"},
        {0x00014217, "npeWinterholdFaction"},
        {0x00039F26, "npeThalmorFaction"},
        {0x000AA0A4, "npeSilverHandFaction"},
    ...
    * Configurable Variables (MCM)
    ...
    float DETECTION_RADIUS = 400.0f;        /// Radius (in units) within which NPC can detect the player
    float FOV_ANGLE = 120.0f;               /// Field of view angle (in degrees)

    bool USE_FOV_CHECK = true;              /// Whether to use field-of-view check (Within the DETECTION_RADIUS)
    bool USE_LINE_OF_SIGHT_CHECK = true;    /// Whether to use line-of-sight check (Within the DETECTION_RADIUS)
    ...
    float GetNPCLevelThreshold() { return NPC_LEVEL_THRESHOLD; }
    void SetNPCLevelThreshold(float v) { NPC_LEVEL_THRESHOLD = v; }
